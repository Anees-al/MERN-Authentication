...
export const register=async(req,res)=>{
  ...
  try{
       ...
       if(existingUser)
        ...
        return res.json({sucess:false,message:"user already existing"});

        }
        const hashedpassword=await bcrypct.hash(password,10);
...
export const login=async(req,res)=>{
  ...
  try{
    ...
    const isMatch= await bcrypct.compare(password,user.password);
    if(!isMatch){
